...
export const setCallArgumentToParameterBound = (services: SafeDsServices) => {
    // upper oder lower bound?
    // acceptor for each option
    // only change single argument, not entire argument list
    ...
    return (diagnostic: Diagnostic, document: LangiumDocument, acceptor: CodeActionAcceptor) => {
        ...
        for (const argument of getArguments(node)) {
            ...
            for (const bound of Parameter.getBounds(param)) {
                ...
                if (bound.operator == '>'){
                    ...
                    acceptor(
                        createQuickfixFromTextEditsToSingleDocument(
                            ...
                            diagnostic,
                            document,
                            [edit], // argList.arguments.flatMap((it) => setToLowerParameterBound(nodeMapper, it)),
                            true,
                        ),
                );
            ...
            /* // upper bound 
                acceptor(
                    createQuickfixFromTextEditsToSingleDocument(
                        'Set argument to upper parameter bound.',
                        diagnostic,
                        document,
                        [edit], // argList.arguments.flatMap((it) => setToLowerParameterBound(nodeMapper, it)),
                        true,
...
const setCallArgumentToUpperParameterBound = (services: SafeDsServices) => {
}

const setCallArgumentToLowerParameterBound = (services: SafeDsServices) => {
    
}

const setToLowerParameterBound = (nodeMapper: SafeDsNodeMapper, argument: SdsArgument): TextEdit[] | TextEdit => {
    const cstNode = argument.$cstNode;
